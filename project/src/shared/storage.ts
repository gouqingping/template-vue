...
export function setup(
	name: any,
	params: any,
	expires: string | number = expiresTime,
): any {
	if (isArray(name)) {
		name.forEach((item: any, i: any) => setup(item, params[i], expires));
		return;
	} else if (isObject(name)) {
		objectEach(name, (i: any, item: string | number) =>
			setup(item, i, expires),
		);
		return;
		...
		Storage.setItem(
			...
			JSON.stringify({
				...
				startTime:
					expires === 'always' ? expires : new Date().getTime(),
			}),
		);
	}
}
...
export function getsub(name: string): any {
	...
	if (item) {
		...
		if (item.startTime != 'always') {
			...
			if (
				isNaN(item.expires) ||
				date - item.startTime > (item?.expires || 0)
			) {
				removeSub(name);
				return false;
			}
