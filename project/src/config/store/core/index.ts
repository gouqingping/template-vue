...
export function createStore<IState = AnyObject>(optons: IStore): IStore {
	...
	if (modules && typeof modules === 'object') {
		Object.keys(modules).forEach((key: string) => {
			...
			if (currentStore) {
				store.state[key] = createPersistStorage<IState>(
					createState(currentStore.state as IStore),
					key,
					true,
				);
				...
				const currentAction: AnyObject = updateAction(
					store.state[key],
					currentStore.actions,
				);
				Object.keys(currentAction).forEach(
					(actionKey: string) =>
						(store.actions[actionKey] = currentAction[actionKey]),
				);
			}
		});
	} else {
		optons &&
			Object.keys(optons).forEach((key: string) => {
